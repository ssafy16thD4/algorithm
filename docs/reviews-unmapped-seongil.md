...
| 파일 | 상태 |
| ---- | ---- |
| `이성일/week6/면접.java` | 대표 풀이 (그리디 구성) |
| `이성일/week6/사과먹기게임.java` | 대표 풀이 (방향 계산) |

> `이성일/week6/starbucks.py` 는 스타벅스 매장 목록을 받아오는 크롤링 스크립트라 알고리즘 풀이가 아닙니다.
> 인덱서가 `.py` 를 제외하므로 리뷰 대상이 아닙니다.

> **문제 지문을 확보하지 못한 상태**에서 코드와 주석만 읽고 쓴 리뷰입니다.
> 아래 검증은 "코드가 가정하는 규칙" 을 그대로 모델링해 대조한 결과입니다.

---

## 1. 면접 — 대표 풀이

**파일**: `이성일/week6/면접.java` / **컴파일**: 통과 / **판정**: good (점수 자료형만 확인 필요)
...
**파일**: `이성일/week6/사과먹기게임.java` / **컴파일**: 통과 / **판정**: wrong

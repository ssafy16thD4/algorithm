...
**주석은 지적 대상이 아니다.** 설명 주석, 계획 주석, 주석 처리된 옛 코드·디버그 출력, 코드와 어긋난 주석 —
전부 개선점에 올리지 않고 `verdict`·태그에도 반영하지 않는다. 채점 서버는 주석을 읽지 않는다.
`dead-code` 는 **실행되는데 쓰이지 않는 변수·함수**에만 붙인다. 주석을 근거로 감점하지 않는다는 건
2026-09-07 팀 결정이다.

**제출 형식도 지적 대상이 아니다 (2026-09-08 팀 결정).** `package` 선언이 남아 있다, 클래스명이 `Main`/`Solution`
이 아니다, 파일명이 문제 제목이다, 안 쓰이는 메서드·변수·import 가 있다 — **전부 개선점·verdict·태그에 올리지
않는다.** IDE 프로젝트 안에서 푼 파일을 그대로 올리는 게 팀 관행이라 거의 모든 파일에 붙어 있고, 제출할 때 각자
고친다. 이걸 지적하면 "패키지 때문에 needs-fix" 같은 판정이 생겨 사람별 통계만 오염된다.

**리뷰에 실을 것은 세 가지뿐이다 — 오답(반례) · 시간초과(복잡도) · 핵심 로직 결함.** 나머지는 쓰지 않는다.
`naming` 태그는 이름이 값을 설명하지 못해 로직을 잘못 읽게 만드는 경우에만, `dead-code` 는 실행 경로에 있어서
결과나 시간에 영향을 주는 군더더기에만 쓴다.
...
- **주석으로 감점하지 않는다.** 주석 처리된 코드가 파일의 절반이어도 개선점·verdict·태그에 올리지 않는다.
  살아 있는 코드만 본다.
- **제출 형식으로 감점하지 않는다.** `package` 선언, 클래스명, 파일명, 미사용 코드는 리뷰에 쓰지 않는다.
  당락을 가르는 것(오답·시간초과·로직 결함)만 쓴다.
